...
func display_weapon(upgrades: Array, max_slots: int, can_max_out: bool):
...
func _input(event):
  if event is InputEventMouseButton:
    if event.button_index == MOUSE_BUTTON_LEFT and event.pressed and $highlight.visible:
      emit_signal("clicked")
